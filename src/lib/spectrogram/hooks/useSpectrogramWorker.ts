...
import SpectrogramWorkerImpl from "../workers/spectrogram.worker.ts?worker";
...
export const useSpectrogramWorker = (
	...
	const requestTileIfNeeded = useCallback(
		async (params: TileGenerationParams) => {
			...
			const requestFingerprint = `${params.tileIndex}-w${params.tileWidthPx}-h${params.height}-g${params.gain}-p${params.paletteId}-z${params.startTime.toFixed(2)}-${params.endTime.toFixed(2)}`;
			...
			const isStale =
				...
				Math.abs(cacheEntry.startTime - params.startTime) > 0.001 ||
				Math.abs(cacheEntry.endTime - params.endTime) > 0.001;
